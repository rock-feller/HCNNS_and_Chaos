...
class HCNNs_modelling_scenario():

    folder_pre_trained = os.makedirs("results/abridged_models/",  exist_ok=True)
    folder_pre_tested = os.makedirs("results/fully_unfolded_models/", exist_ok=True)
    ...
    def model_name( self , hcnn_instance  )-> str:
        ...
        if hcnn_instance.name == "ptf":
            model_full_name = f"{self.simulation_id}:_{model_type}:_hd:{hidden_vars}{hcnn_instance.train_s0}_ctxt:{self.context_window_size}_fc:{fcast_hor}steps_TargetProb:{hcnn_instance.target_prob}::_{self.num_epochs}epochs_trained_on_{date.today().strftime('%d_%m')}"
        elif hcnn_instance.name == "largesparse":
            
            model_full_name = f"{self.simulation_id}:_{model_type}:_hd:{hidden_vars}{hcnn_instance.train_s0}_ctxt:{self.context_window_size}_fc:{fcast_hor}steps_TargetProb:{hcnn_instance.prop_zeroed_weights}::_{self.num_epochs}epochs_trained_on_{date.today().strftime('%d_%m')}"
        else:
    ...
    def results_saver(self, hcnn_instance, results_dictionary ):
        ...
        pd.DataFrame(results_dictionary['pred_testdata'], columns=['pred_x', 'pred_y', 'pred_z']).to_csv(f"{output_csv}/{hcnn_instance.__class__.__name__}_{self.simulation_id}_pred.csv" , index=False)
        pd.DataFrame(results_dictionary['true_testdata'], columns=['true_x', 'true_y', 'true_z']).to_csv(f"{output_csv}/{hcnn_instance.__class__.__name__}_{self.simulation_id}_true.csv" , index=False)
    ...
    def test_only( self, hcnn_instance:nn.Module , context_window:torch.Tensor, forecast_horizon :int ) -> Tuple[torch.Tensor]:
        ...
        preds , fs_states = hcnn_instance.forecast( initial_state, context_window , forecast_horizon)
    ...
    def fully_unfolded(self, hcnn_instance:nn.Module, train_data : torch.Tensor ,true_testdata : torch.Tensor , 
        ...
        self.set_seed(42)
        folder_fullyunf = "results/fully_unfolded_models/"
        #device  = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    def abridged_mode(self, hcnn_instance: nn.Module,  train_loader ,context_window:torch.Tensor, num_epochs:int, 
        ...
        training_losses = []
        self.set_seed(42)
        folder_abridged = "results/abridged_models/"
        prob  = 0.
        for epoch in range(num_epochs):
            ...
            for  batch_id, batch_data in enumerate( train_loader):
                ...
                out_target = torch.zeros_like(batch_data , dtype=torch.float32)
                ...
                pred_testdata , fs_states = self.test_only( hcnn_instance, context_window.unsqueeze(1) , forecast_horizon)

                test_loss = criterion(pred_testdata[-forecast_horizon:], true_testdata.unsqueeze(1)).item()#test_data
                ...
                if test_loss < best_test_loss:
                    ...
                    if hcnn_instance.name =="lstm_form":
                        results_dict.update({"diag_params": hcnn_instance.state_dict()['cell.D.weight'].diag().numpy()})
        ...
        results_dict['true_testdata'] = true_testdata.squeeze(1).detach().cpu().numpy()
        results_dict["model_name:"]= self.model_name(hcnn_instance)
        torch.save(deepcopy(results_dict["model_"]) , f"{folder_abridged}Te_{self.model_name(hcnn_instance)}.pt")
