...
class ptf_cell(nn.Module):
    ...
    def __init__(self, n_obs: int, n_hid_vars: int ):#, prob: float):#, drop_output: bool): #, s0_nature: str, train_s0: bool
    ...
    def forward(self, state: torch.Tensor,  prob:float, allow_transition :bool  , drop_output : bool , 
        ...
        if observation is not None:
            ...
            if allow_transition == True:
                ...
                next_state = self.A( torch.tanh(r_state)) #+ torch.matmul(self.B, external)
...
class vanilla_cell(nn.Module):
    ...
    def forward(self, state: torch.Tensor, allow_transition :bool ,
                 observation: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def forward(self, state: torch.Tensor, allow_transition :bool ,
        ...
        if observation is not None:
            ...
            if allow_transition == True:
                ...
                next_state =  r_state   +  self.D( lstm_block ) #+ torch.matmul(self.B, external)
...
class mask_for_sparsity:
    def __init__(self, n_hid_vars:int, n_obs:int , prop_zeroed_weights:float):
        self.n_obs = n_obs
        self.n_hid_vars = n_hid_vars
    ...
    def split_design( self ) -> torch.Tensor:
        ...
        zeroed_weights =  int(self.prop_zeroed_weights* total_weights )
...
class LargeSparse_cell(nn.Module):
    ...
    device  = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    dtype = torch.float32 
    def __init__(self, n_obs: int, n_hid_vars: int, prop_zeroed_weights:float):
        super(LargeSparse_cell, self).__init__()
        self.n_obs = n_obs
        self.n_hid_vars = n_hid_vars
        self.prop_zeroed_weights = prop_zeroed_weights
        sparse_inits =  mask_for_sparsity(n_hid_vars=self.n_hid_vars , n_obs=self.n_obs, prop_zeroed_weights=self.prop_zeroed_weights)
        #self.dtype = torch.float32 
        #self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    def forward(self, state: torch.Tensor, allow_transition :bool ,
        ...
        if observation is not None:
            ...
            if allow_transition == True:
                ...
                next_state = self.Sparse_A( torch.tanh(r_state)) #+ torch.matmul(self.B, external)
